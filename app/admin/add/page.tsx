...
import { Users, Home, Trash2, Camera, UserPlus, ShieldCheck, Crown } from 'lucide-react';
            ...
            {activeTab === 'properties' ? (
              <form onSubmit={handleAddProp} className="space-y-4">
                <div className="aspect-video bg-black rounded-2xl border border-white/10 flex flex-col items-center justify-center cursor-pointer overflow-hidden group hover:border-[#D4AF37] transition-all" onClick={() => fileRef.current?.click()}>
                  ...
                  <><Camera className="w-8 h-8 text-[#D4AF37] opacity-50"/><p className="text-[10px] font-bold text-gray-500 mt-2 uppercase">Property Visual</p></>}
                ...
                <button className="w-full py-5 bg-gradient-to-r from-[#D4AF37] to-[#B8860B] text-black rounded-xl font-black uppercase text-xs tracking-[0.2em]">Publish to Portfolio</button>
              </form>
            ) : (
              <form onSubmit={handleAddAgent} className="space-y-4">
                ...
                <button className="w-full py-5 bg-[#D4AF37] text-black rounded-xl font-black uppercase text-xs tracking-[0.2em]">Authorize Agent</button>
              </form>
            )}
          </section>
              ...
              {(activeTab === 'properties' ? properties : agents).map((item: any) => (
                <div key={item.id} className="group bg-[#111] p-4 rounded-2xl border border-white/5 flex items-center justify-between hover:border-[#D4AF37]/30 transition-all">
                  <div className="flex items-center gap-4">
                    <img src={item.images ? item.images[0] : item.image} className={`w-14 h-14 object-cover ${activeTab === 'agents' ? 'rounded-full' : 'rounded-xl'} border border-white/10`} alt="" />
                    <div>
                      <p className="font-bold text-gray-200 text-sm tracking-tight">{item.title || item.name}</p>
                      <p className="text-[9px] text-[#D4AF37] font-black uppercase tracking-widest">{item.city || item.role}</p>
                    </div>
                  </div>
