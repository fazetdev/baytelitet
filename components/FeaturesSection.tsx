...
import { Calculator, Home, Globe, Settings, ShieldCheck } from 'lucide-react';
...
const features: Feature[] = [
  ...
  },
  {
    ...
    titleEn: 'Agent Command Center',
    titleAr: 'مركز قيادة الوكيل',
    descriptionEn: 'Lead intelligence, RERA compliance, and deal pipeline',
    descriptionAr: 'ذكاء العملاء، امتثال ريرا، ومتابعة الصفقات',
            ...
            <Link key={feature.id} href={feature.href} className="group block">
              <div className={`bg-white rounded-2xl shadow-lg p-8 border border-gray-100 hover:border-bayt-warm hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 h-full`}>
                ...
                <div className="text-bayt-warm font-semibold flex items-center gap-2">
                  ...
                  <span className={`${isRTL ? 'group-hover:-translate-x-2' : 'group-hover:translate-x-2'} transition-transform`}>
                    {isRTL ? '←' : '→'}
                  </span>
                </div>
              </div>
