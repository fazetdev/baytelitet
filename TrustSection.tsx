        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {trustItems.map((item) => (
            ...
            >
              ...
              {/* Proof / Link */}
              {item.proofLink && item.proofLabelEn && item.proofLabelAr ? (
                <a
                  key={`${item.id}-proof`}
                  href={item.proofLink}
                  target="_blank"
                  ...
                  aria-label={isRTL ? item.proofLabelAr : item.proofLabelEn}
                >
                  {isRTL ? item.proofLabelAr : item.proofLabelEn}
                </a>
              ) : item.proofLabelEn && item.proofLabelAr ? (
                <span
                  key={`${item.id}-proof`}
                  className="inline-block bg-bayt-warm text-bayt-dark text-xs px-2 py-1 rounded-full"
                >
                  {isRTL ? item.proofLabelAr : item.proofLabelEn}
                </span>
              ) : null}
